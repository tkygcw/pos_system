...
class PaymentReceiptLayout {
  ...
  Future<List<int>> print80mmFormat(bool isUSB, {value}) async {
    ...
    try{
      ...
      bytes += generator.hr();
      bytes += generator.text('Payment Sales', containsChinese: true, styles: PosStyles(align: PosAlign.center, bold: true));
      ...
      bytes += generator.hr();
      bytes += generator.row([
        PosColumn(text: 'Payment', width: 5, styles: PosStyles(bold: true, align: PosAlign.left)),
      ...
      for(final detail in model.reportValue2){
        bytes += generator.row([
          PosColumn(text: detail.payment_name, width: 5, containsChinese: true),
  ...
  Future<List<int>> print58mmFormat(bool isUSB, {value}) async {
    ...
    try{
      ...
      bytes += generator.hr();
      bytes += generator.row([
        PosColumn(text: 'Payment', width: 5, styles: PosStyles(bold: true)),
        PosColumn(text: 'Qty', width: 3, styles: PosStyles(bold: true)),
