...
class DiningReceiptLayout {
  ...
  Future<List<int>> print80mmFormat(bool isUSB, {value}) async {
    ...
    try{
      ...
      bytes += generator.hr();
      bytes += generator.text('Dining Sales', containsChinese: true, styles: PosStyles(align: PosAlign.center, bold: true));
      ...
      bytes += generator.hr();
      bytes += generator.row([
        PosColumn(text: 'Dining', width: 5, styles: PosStyles(bold: true, align: PosAlign.left)),
      ...
      for(final detail in model.reportValue2){
        bytes += generator.row([
          PosColumn(text: detail.dining_name, width: 5, containsChinese: true),
