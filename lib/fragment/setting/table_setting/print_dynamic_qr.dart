...
import 'package:pos_system/fragment/setting/table_setting/dynamic_qr_layout.dart';
...
class PrintDynamicQr {
  ...
  Future<List<Printer>> readCashierPrinter() async {
    List<Printer> data = await PosDatabase.instance.readAllBranchPrinter();
    return data.where((e) => e.is_counter == 1 && e.printer_status == 1).toList();
  }

  printDynamicQR({required List<PosTable> tableList}) async {
    List<Printer> cashierPrinter = await readCashierPrinter();
